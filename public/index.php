...
$publicActions = array('home', 'login', 'authenticate', 'logout', 'forgot', 'forgot.send', 'auth.verify');
    ...
    switch ($definition['action']) {
        ...
        case 'authenticate':
            ...
            $_SESSION['auth'] = array(
                ...
                'role'  => $user->rol(),   // getter correcto en este proyecto
            );
        ...
        case 'auth.verify':
            ...
            try {
                $verifyService = DependencyInjection::getVerifyEmailService(); // Configura esto en tu DI
                $verifyService->execute($email, $token);

                Flash::setSuccess('¡Tu correo ha sido verificado! Ya puedes iniciar sesión.');
            } catch (Throwable $e) {
                Flash::setMessage('Error en la verificación: ' . $e->getMessage());
            }
...
function buildCreateUserViewData(): array
{
    return array(
        ...
        'roleOptions' => UserRolEnum::values(),   // nombre real del enum en este proyecto
        'message'     => Flash::message(),
        'success'     => Flash::success(),
        'errors'      => Flash::errors(),
        'old'         => Flash::old(),
...
function validateUpdateUserForm(array $form): array
{
    ...
    // Contraseña opcional en edición: solo validar si se proporcionó.
